...
class HL7::CompositeField
  ...
  def initialize(value='')
    @element_delim = '^'    
    @elements=[self.class.to_s]+value.split(@element_delim, -1)
  end

  # provide a screen-readable version of the message
  public
  def to_s    
    #@elements[1,@elements.length].collect { |s| s if s.to_s.length > 0 }.join( "\n" )
    to_hl7
  end

  # provide a HL7 spec version of the message
  def to_hl7
    @elements[1,@elements.length].collect { |s| s if s.to_s.length > 0 }.join( @element_delim ) 
  end
